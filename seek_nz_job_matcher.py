...
Automated SEEK New Zealand job-search assistant.

This script:
1. Opens a SEEK New Zealand search-results page with Selenium.
...
BASE_URL = "https://nz.seek.com/Python,-Automation,-entry,-graduate,-Early,-Junior-jobs"
SEEK_DOMAIN = "https://www.seek.co.nz"

OLLAMA_URL = "http://localhost:11434/api/generate"
MODEL = "gpt-oss:20b"

DEFAULT_LOCATION = "New Zealand"
DEFAULT_PAGES = 9999999
DEFAULT_PER_PAGE = 0
DEFAULT_OUT_CSV = "seek_nz_matches.csv"
DEFAULT_OUT_JSONL = "seek_nz_full_results.jsonl"

PAGE_DELAY_SECONDS = 1.0
JOB_DELAY_SECONDS = 0.5

LOCATION_REGEX = r"\b(Auckland|Wellington|Christchurch|Hamilton|Tauranga|Dunedin|Canterbury|Otago|Waikato|Northland|Taranaki|Southland|Nelson|Rotorua|Whangarei|Invercargill|Queenstown|Marlborough|Gisborne|Tasman|West\s*Coast)\b"

SYSTEM_PROMPT = 'You are a job-matching assistant.\n\nReturn ONLY one JSON object exactly like:\n{"suitable": true/false, "reason": "<中文一句话原因>"}\n\nCandidate facts (must assume true):\n- The candidate is a Chinese citizen.\n- The candidate does NOT currently hold New Zealand work rights.\n- The candidate would require employer sponsorship to work full-time in New Zealand.\n- The candidate is NOT a New Zealand citizen and NOT a New Zealand permanent resident.\n\nRules:\n- Read the JOB description and the CANDIDATE profile.\n- Consider jobs related to computer science/software engineering, Python programming, algorithms, data engineering/analysis, machine learning (optional), computer vision (optional), automation/control systems, embedded systems, electronics, robotics, and interdisciplinary roles combining these areas.\n- WORK RIGHTS FILTER IS STRICT: If the job explicitly requires New Zealand Citizen / Permanent Resident (PR) / Citizenship / PR Only / current NZ work rights / no sponsorship, then set "suitable": false.\n- YEARS OF EXPERIENCE REQUIREMENTS ARE STRICT. If the job explicitly requires X+ years of professional experience and the candidate does not have that experience, set "suitable": false.\n- DEFENCE FILTER IS STRICT: If the job is related to defence, military, weapons, surveillance for defence use, national security, defence contractors, or requires defence clearance / security clearance for defence-related work, then set "suitable": false.\n- If the job does NOT explicitly require years of experience, then evaluate whether the candidate can reasonably perform the responsibilities based on skills, academic background, internships, and projects.\n- Consider required skills, technical stack if explicitly stated.\n- Output MUST be valid JSON only, with exactly two keys: "suitable" and "reason".\n- The "reason" must be in Chinese, concise (<= 20 Chinese characters preferred, <= 30 max), and mention the main deciding factor.\n- No extra fields, no markdown, no chit-chat.'
...
DEFAULT_ABOUT_ME = 'I am a recent graduate with a Master of Computing and Innovation degree in Australia with a qualification recognised in New Zealand. I do not currently hold New Zealand work rights and would require employer sponsorship to work full-time in New Zealand. I hold a clean background and I am licensed to drive in New Zealand. I also hold a Bachelor’s degree in Electrical Engineering and Intelligent Control, with foundations in PLC programming, PCB design, and system simulation using MATLAB and Simulink. Although I do not yet have formal full-time work experience, I completed internships that gave me exposure to finance, engineering, and robotics. At Haitong Securities, I completed a 6-month rotation internship involving data analysis, securities trading, and venture capital exposure. At China Galaxy Securities, I completed a 2-month internship focused on financial analysis. At Thyssenkrupp, I supported engineering-related testing and control-system improvement tasks. At MicroPort MedBot, I worked as an R&D intern in the Flexible Endoscopy Department, where I modelled and optimised catheter robot control systems using Simulink and STM32. I also completed a university project on a PLC-based self-service pricing and heating system, where I independently designed control schemes, implemented PLC ladder logic, integrated an ATmega16 subsystem, and conducted hardware and simulation testing. My technical skills include Python, PyTorch, computer vision, machine learning, deep learning, image and data processing, automation, model evaluation, MATLAB, Simulink, embedded C, PLC control, SolidWorks, and engineering documentation. These experiences demonstrate my ability to combine computing, engineering, automation, and data analysis. I am eager to start my professional career in machine learning, computer vision, applied AI engineering, automation, software workflows, data processing, or engineering technology.'
...
def parse_args() -> argparse.Namespace:
    ...
    parser = argparse.ArgumentParser(
        description="Filter SEEK New Zealand jobs with Selenium and a local Ollama model."
    )
